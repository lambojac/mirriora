...
export const register = async (body: RegisterBody, res: Response) => {
  ...
  try {
    // 🔍 Check for existing users (email / phone)
    if (email) {
    ...
    if (error) {
      ...
      return { status: 500, data: { message: "Failed to create user account." } };
    }
